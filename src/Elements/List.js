...
const ListItem = ({ id }) => {
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
          e.preventDefault(); // Prevent default new line creation
          ...
          // Find the index of the current item in the parent's children list
          ...
          // Add a new element right after the current one
          const newId = addNewElement('list-item', 1, null, parentId);
      
          // Update the parent's children to include the new element at the correct position
          ...
          // Set the newly added element as the selected one for immediate editing
  ...
  const List = ({ id, type = 'ul' }) => {
    ...
    useEffect(() => {
      if (listElement && listElement.children.length === 0) {
        // Only add a new item if no children exist (and only once)
